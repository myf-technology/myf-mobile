...
export const cpfcnpjValidations = async ({ cnpj, document }: registerDocumentPayload) => {
  ...
  const DOCUMENT_INVALID_SIZE = DOCUMENT_LENGTH > CPF_LENGTH && DOCUMENT_LENGTH < CNPJ_LENGTH;
...
export const fullName = ({ name }: registerNameBirthdatePayload) => {
  ...
  if (!FIRST_NAME_LENGTH) return 'Primeiro nome precisa conter ao menos 3 dígitos.';
