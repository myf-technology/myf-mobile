...
type Props = {
   ...
   * Might be number or percent
   * (as string e.g. '20%') for points or percents of screen height from bottom.
   */
  ...
  /**
   * Reanimated node which holds position of bottom sheet,
   * where 1 it the highest snap point and 0 is the lowest.
   */
  ...
  /**
   * Defines how violently sheet has to stopped
   * while overdragging. 0 means no overdrag. Defaults to 0.
   */
...
export default class WithModalBehaviour extends React.Component<Props, State> {
  static defaultProps = {
  ...
  constructor(props: Props) {
    ...
    this.state = WithModalBehaviour.getDerivedStateFromProps(props, undefined);
  ...
  snapTo = (index: number) => {
    ...
    this.manuallySetValue.setValue(
      this.state.snapPoints[this.state.propsToNewIndices[index]] as any
    );
  };
      ...
      .map(
        (
          ...
          if (typeof s === 'number') {
            ...
            return { val: WithModalBehaviour.renumber(s), ind: i };
          }
